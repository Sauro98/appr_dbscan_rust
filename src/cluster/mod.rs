...
pub fn assign_border_noise_points<const D: usize>(cells: &CellTable<D>, points: &Vec<Point<D>>,clusters: &mut DBSCANResult, params: &DBSCANParams) {
    for cell in cells.values() {
        for s_point in cell.points.iter().filter(|x| !x.is_core) {
            for neighbour in cell.neighbour_cell_indexes.iter()
                                                        .map(|n_i| cells.get(n_i).unwrap())
                                                        .filter(|n| n.is_core) {
                if neighbour.core_info.root.approximate_range_counting_root(
                    &points[s_point.point_index],&params
                ) != 0 {
                    clusters[s_point.point_index] = Some(neighbour.core_info.i_cluster);
