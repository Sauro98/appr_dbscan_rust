...
pub fn points_in_range<const D: usize>(point_i: usize, points: &Vec<Point<D>>,cell: &Cell<D>, epsilon: f64) -> usize{
    let mut cnt : usize = 0;
    for s_point in &cell.points {
        if euclidean_distance(&points[point_i], &points[s_point.point_index]) <= epsilon {
            cnt += 1;
        }
    }
    cnt
...
pub fn label_points<const D: usize>(cells: &mut CellTable<D>, points: &Vec<Point<D>>, params: &DBSCANParams) -> PartitionVec<CellIndex<D>> {
    ...
    for cell in cells.values_mut() {
        if cell.points.len() >= params.min_pts {
            label_dense_cell(cell, points, params, &mut part_vec);
        } else {
            label_sparse_cell(&cells_cloned, cell, points, params, &mut part_vec)
        }
    }
    part_vec
}
...
fn label_dense_cell<const D: usize>(cell: &mut Cell<D>, points: &Vec<Point<D>>, params: & DBSCANParams, uf_str: &mut PartitionVec<CellIndex<D>>){
    cell.is_core = true;
    cell.core_info.uf_index = uf_str.len();
    let points : Vec<Point<D>> = cell.points.iter().map(|x| points[x.point_index]).collect();
...
/// memorized in the cell
fn label_sparse_cell<const D: usize>(cells_c: &CellTable<D>,curr_cell: &mut Cell<D>, points: &Vec<Point<D>>, params: &DBSCANParams, uf_str: &mut PartitionVec<CellIndex<D>>){
    let len = curr_cell.points.len();
    let mut core_points : Vec<Point<D>> = Vec::with_capacity(curr_cell.points.len());
    for mut s_point in &mut curr_cell.points {
        ...
        for n_index in &curr_cell.neighbour_cell_indexes {
            if !is_same_index(&curr_cell.index, n_index) {
                ...
                tot_pts += points_in_range(s_point.point_index, points, neighbour, params.epsilon);
            }
        ...
        if tot_pts >= params.min_pts {
            ...
            core_points.push(points[s_point.point_index].clone());
        }
    }
    if curr_cell.is_core {
        curr_cell.core_info.uf_index = uf_str.len();
        uf_str.push(curr_cell.index.clone());      
        curr_cell.core_info.root = TreeStructure::build_structure(core_points, params);
    }
}
...
pub fn compute_adjacency_lists<const D: usize>(cells:  &mut CellTable<D>, points: &Vec<Point<D>>, params: &DBSCANParams, part_vec: &mut PartitionVec<CellIndex<D>>){
        ...
        for n_index in &cell.neighbour_cell_indexes {
            ...
            if neighbour.is_core {
                ...
                for point in cell.points.iter().filter(|p| p.is_core) {
                    if neighbour.core_info.root.approximate_range_counting_root(&points[point.point_index], params) != 0 {
