...
pub fn approximate_dbscan<const D: usize>(points: &Vec<Point<D>>, params: &DBSCANParams) -> DBSCANResult {
    let tot = Instant::now();
    //let now = Instant::now();
    ...
    //let now = Instant::now();
    let mut part_vec = label_points(&mut base_cells, points, params);
    //println!("Found {} core cells in {} ms",base_cells.values().filter(|x| x.is_core).count(),now.elapsed().as_millis());
    //let now = Instant::now();
    compute_adjacency_lists(&mut base_cells, points, params, &mut part_vec);
    //println!("Graph built in {} ms",now.elapsed().as_millis());
    //let now = Instant::now();
    let mut result = find_connected_components(&mut base_cells, points, part_vec);
    //println!("Found {} clusters in {} ms",result.len() - 1,now.elapsed().as_millis());
    //let now = Instant::now();
    assign_border_noise_points(&base_cells, points, &mut result, params);
